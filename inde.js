...
select.addEventListener("change", () => {
  if (select.value === "hl") {
    ...
    appenddata(newfood);
  } else if (select.value === "sd") {
    let newfood = [...foodData];
    newfood.foodData.sort((a, b) => {
      const costA = parseFloat(a.distance.replace("km", ""));
      const costB = parseFloat(b.distance.replace("km", ""));
      return costA - costB;
    });
    appenddata(newfood);
  } else if (select.value === "sr") {
    let newfood = [...foodData];
    newfood.sort((a, b) => {
      const costA = parseFloat(a.starRating);
      const costB = parseFloat(b.starRating);
      return costB - costA;
    });
    appenddata(newfood);
  } else if (select.value === "sf") {
    let newfood = [...foodData];
    newfood.sort((a, b) => a.foodName.localeCompare(b.foodName));
    appenddata(newfood);
  ...
  newfood.forEach((element) => {
    ...
    div.innerHTML = `
                <img src=${element.img} alt="">
                    ...
                    <p><b>Star Rating :</b> ${element.starRating}</p>
                    <p><b>Foods:</b> ${element.foods.join(",")}</p>
                    <p><b>Place :</b> ${element.place}</p>
                    <p><b>Distance  :</b> ${element.distance}</p>
                    <p><b>Price :</b> ${element.cost}</p>
                `;
